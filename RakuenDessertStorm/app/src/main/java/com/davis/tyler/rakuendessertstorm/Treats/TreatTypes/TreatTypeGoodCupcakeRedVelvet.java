...
public class TreatTypeGoodCupcakeRedVelvet implements ITreatTypeGood {
    private Bitmap bitmap;
    private final int RECT_HEIGHT ;
    private final int RECT_WIDTH ;
    private int pointValue;
    private int type = Treat.CUPCAKE_RED_VELVET;

    public TreatTypeGoodCupcakeRedVelvet(Context context){
        RECT_HEIGHT = (int)(Constants.SCREEN_HEIGHT * .119);
        RECT_WIDTH = (int)(RECT_HEIGHT * .8);
        bitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.cupcakeredvelvet);
        pointValue = 50;
